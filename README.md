...
## Environment
The environment is an adaptation of Unity's ML-Agents "Dungeon Escape" environment, that was **rebuilt from scratch** (code and assets) to fit the needs of the project. The environment consists of a square arena with 5 fixed pillars, a door, a cave, and a user selected number of agents and dragons. The dragons always move towards the cave move using the $A^*$.
